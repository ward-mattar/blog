...
בתמונה הנוכחית צבעתי את הפאות בצבעים עליזים - ורוד, ירוק, כחול, צהוב ולבן. רגע, איפה הלבן כאן? ובכן, גם השטח ש"מחוץ" לגרף נחשב לאחת מהפאות שלו, "הפאה החיצונית". כלומר, הצביעה העליזה שלי מראה לנו שלגרף יש חמש פאות. בנוסף לכך יש לו חמישה צמתים (הנקודות הכחולות) ויש לו שמונה קשתות (הקווים השחורים העבים שמחברים צמתים). אם נסמן את מספר הצמתים ב-{::nomarkdown}\(V\){:/nomarkdown}, את מספר הקשתות ב-{::nomarkdown}\(E\){:/nomarkdown} ואת מספר הפאות ב-{::nomarkdown}\(F\){:/nomarkdown} נגלה שמתקיים

{::nomarkdown}\(V-E+F=2\){:/nomarkdown}

והקשר הזה בין מספר הצמתים, הקשתות והפאות מתקיים <strong>תמיד</strong>, בכל גרף מישורי קשיר; זו נוסחת אוילר, שהיא ללא ספק אחת מהנוסחאות החביבות עלי במתמטיקה. המטרה שלי בפוסט הזה היא להוכיח שהיא אכן מתקיימת תמיד, בכל גרף מישורי קשיר (חייבים קשירות כי אחרת אפשר להגדיל את מספר הצמתים {::nomarkdown}\(V\){:/nomarkdown} בלי להשפיע על מספר הקשתות או הפאות, סתם על ידי הוספת צמתים מבודדים), אבל <strong>הוכחה</strong> היא מילה די גדולה בהקשר הזה - שנתפשר על "המחשה יפה"?

רגע, למה בעצם אני לא יכול לתת הוכחה של ממש? ובכן, כל המושג של "גרף מישורי" הוא די קשה מבחינה פורמלית. מה זה אומר "לצייר את הגרף במישור כך שאין חיתוכים"? צריך לערב כאן עולם מושגי חדש לגמרי - המישור {::nomarkdown}\(\mathbb{R}^{2}\){:/nomarkdown} ומה זה אומר "לצייר" בו משהו - זה אומר ליצור עקומות; עקומה היא פונקציה רציפה {::nomarkdown}\(f:\left[0,1\right]\to\mathbb{R}^{2}\){:/nomarkdown}, ועכשיו צריך לדבר על חיתוך של עקומות, ועל מה זו בעצם פאה - רכיב קשירות נפרד של המישור כשמסירים ממנו את האיזורים שמכוסים על ידי עקומות? כל הדברים הללו מובילים אותנו אל משפט מפלצתי שנקרא <strong>משפט ז'ורדן </strong>ואומר (בנפנוף ידיים) שאם מציירים עיגול במישור אז הוא מפרק את המישור ל"בפנים" וה"בחוץ" של העיגול. זה משפט קשה עם הוכחה קשה. אז לא, אני לא אכנס לפורמליזם הזה פה ואפילו לא אנסה - בדיוק בשביל זה הבלוג נקרא "לא מדויק".

הרעיון ב"הוכחה" שלי הוא פשוט ויפה וניתן לתיאור בפסקה אחת לפני שנדגים אותו בצורה מפורטת. אנחנו נצייר את הגרף המישורי שלנו מבלי להרים את העט מהדף, ונעקוב כל הזמן אחרי מספר הפאות הנוכחי. כל "צעד" שלנו כולל ציור של קשת אחת, ולכן אחרי {::nomarkdown}\(E\){:/nomarkdown} צעדים הציור יושלם. כשאנחנו רק מתחילים והציור ריק, יש לנו רק פאה אחת - הפאה החיצונית. מתי נוצרות פאות חדשות? בדיוק בצעדים שמחזירים אותנו לצומת שבה כבר היינו. כמה צעדים כאלו יש? אם יש {::nomarkdown}\(E\){:/nomarkdown} צעדים בסך הכל ולכן אנחנו מבקרים בסך הכל ב-{::nomarkdown}\(E+1\){:/nomarkdown} צמתים במהלך הציור - הפלוס אחד מגיע מכך שאנחנו נמצאים בצומת כלשהי כבר בהתחלה, עוד לפני שציירנו קשת אחת. יש בסך הכל {::nomarkdown}\(V\){:/nomarkdown} צמתים ולכן אנחנו מבקרים ב-{::nomarkdown}\(E+1-V\){:/nomarkdown} צמתים יותר מפעם אחת. לכן מספר הפאות <strong>שמתווספות</strong> אל האחת הראשונית יהיה {::nomarkdown}\(E+1-V\){:/nomarkdown}, כלומר {::nomarkdown}\(F-1=E+1-V\){:/nomarkdown}, ואחרי העברת אגפים נקבל {::nomarkdown}\(V-E+F=2\){:/nomarkdown}.
...
בואו נעקוב אחרי תהליך הציור של התמונה לעיל, עם ספירה של מספר הקשתות והפאות בכל רגע נתון. כל חמשת הצמתים יופיעו כל הזמן - כלומר, {::nomarkdown}\(V=5\){:/nomarkdown} הולך להיות קבוע. בהתחלה יש רק את הצמתים, ודמיינו שאנחנו עומדים בצומת הימני-תחתון ומחכים בהתרגשות לתחילת המסע שלנו בגרף:
...
כאן {::nomarkdown}\(E=0\){:/nomarkdown} ו-{::nomarkdown}\(F=1\){:/nomarkdown} (כי הפאה החיצונית, הלבנה).
...
ועכשיו {::nomarkdown}\(E=1\){:/nomarkdown} ו-{::nomarkdown}\(F=1\){:/nomarkdown} נשאר כמו קודם - מכיוון שהקשת הגיעה לצומת שטרם ביקרנו בו, לא יצרנו פאה חדשה. אנחנו רואים שלבקר בצומת שטרם ביקרנו בו זה משעמם, אז בואו נבצע את כל יתר הצעדים הנדרשים כדי שנבקר <strong>בכל</strong> הצמתים; מרגע זה והלאה, כל צעד נוסף יצור פאה חדשה:
...
כרגע {::nomarkdown}\(E=4\){:/nomarkdown} (כמספר הצמתים פחות אחד - לא מקרי, כמובן, הקשר {::nomarkdown}\(E=V-1\){:/nomarkdown} מתקיים בכל <strong>עץ</strong>) ו-{::nomarkdown}\(F=1\){:/nomarkdown}. כלומר, כבר כרגע מתקיימת הנוסחה {::nomarkdown}\(V-E+F=2\){:/nomarkdown}, וזה לא מפתיע כי הגרף שקיבלנו כרגע הוא גם מישורי וגם <strong>קשיר</strong> (זוכרות? קשירות הייתה הכרחית בשביל נוסחת אוילר, בדיוק בגלל שצמתים מבודדים כמו בתמונות הקודמות לא משפיעים על {::nomarkdown}\(E\){:/nomarkdown} ועל {::nomarkdown}\(F\){:/nomarkdown} ואפשר להוסיף כמה מהם שרוצים). מה שיקרה מכאן ואילך הוא שכל צעד <strong>ישמר</strong> את הקשר הזה, {::nomarkdown}\(V-E+F=2\){:/nomarkdown}; כי כל צעד גם יוסיף קשת אחת (יגדיל את {::nomarkdown}\(E\){:/nomarkdown} ב-1) וגם יצור פאה חדשה (יגדיל את {::nomarkdown}\(F\){:/nomarkdown} ב-1). בואו נראה את זה קורה בפעם הראשונה:
...
הגרף הימני הוא הגרף המפורסם של "בעיית הגשרים של קניגסברג" שאוילר פתר. כל הצמתים שם מדרגה אי זוגית ולכן הגרף אינו אוילרי, למרות שהוא בבירור מישורי ובבירור מקיים את נוסחת אוילר ({::nomarkdown}\(V=4\){:/nomarkdown} ו-{::nomarkdown}\(E=7\){:/nomarkdown} ו-{::nomarkdown}\(F=5\){:/nomarkdown}). הגרף השמאלי הוא מה שקורה לגרף הימני כשאני לוקח כל קשת ומפצל אותה לשתיים. ההשפעה של הפיצול הזה על הגרף היא זו:
<ol>
...
{::nomarkdown}\(V-2E+\left(F+E\right)=2\){:/nomarkdown}

כלומר {::nomarkdown}\(V-E+F=2\){:/nomarkdown}, שזה מה שרצינו להוכיח, וסיימנו.
