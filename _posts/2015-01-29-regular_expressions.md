...
בואו נסתכל למשל בשפה {::nomarkdown}\( L=\left\{ a^{n}b^{k}\ \|\ n,k\in\mathbb{N}\right\} \){:/nomarkdown}. זו שפה רגולרית, ואפשר להציג אותה בתור {::nomarkdown}\( \left\{ a\right\} ^{*}\cdot\left\{ b\right\} ^{*}\){:/nomarkdown}. אבל שתי דרכי הכתיבה שראינו עד כה הן מסורבלות - הן כוללות סימנים מיותרים. למה לא לכתוב {::nomarkdown}\( a^{*}b^{*}\){:/nomarkdown} וחסל? ובכן, זה בדיוק מה שנעשה. {::nomarkdown}\( a^{*}b^{*}\){:/nomarkdown} היא דוגמה פשוטה לביטוי רגולרי - בסך הכל שיטה נוחה לכתיבת שפות מסויימות, שיש בצידה בונוס רציני: כל שפה שמיוצגת על ידי ביטוי רגולרי היא אוטומטית שפה רגולרית, ואנחנו יודעים שכל שפה רגולרית יהיה ניתן לייצג בצורה כזו.

הפורמליזם פשוט: איחוד מסמנים עם {::nomarkdown}\( +\){:/nomarkdown} ולא עם {::nomarkdown}\( \cup\){:/nomarkdown}, שרשור מסמנים עם {::nomarkdown}\( \cdot\){:/nomarkdown}, ושמים סוגריים על הכל; אבל בפועל משמיטים סוגריים כאשר סדר הקדימויות הוא המתבקש (קודם {::nomarkdown}\( *\){:/nomarkdown}, אחר כך {::nomarkdown}\( \cdot\){:/nomarkdown} ולבסוף {::nomarkdown}\( +\){:/nomarkdown}) ומשמיטים את {::nomarkdown}\( \cdot\){:/nomarkdown} כי לא צריך. לכן כותבים {::nomarkdown}\( a^{*}b^{*}\){:/nomarkdown} ולא {::nomarkdown}\( \left(\left(a^{*}\right)\cdot\left(b^{*}\right)\right)\){:/nomarkdown}. בכל זאת, הנה הפורמליזם המדויק. הוא כולל שני חלקים - <strong>תחביר (סינטקס)</strong> ו<strong>סמנטיקה</strong>. התחביר אומר לנו אילו מחרוזות של סימבולים הן ביטויים רגולריים חוקיים; הסמנטיקה אומרת לנו מה השפה שביטוי רגולרי חוקי נתון מגדיר (די דומה להגדרות שיש בלוגיקה, למשל בתחשיב הפסוקים - שם התחביר מגדיר מהי נוסחה לוגית חוקית, והסמנטיקה אומרת מה ערך האמת שלה בתוך הקשר מסויים).

כרגיל בדיונים על שפות רגולריות יש ברקע איזה אלפבית {::nomarkdown}\( \Sigma\){:/nomarkdown} שכל המילים נבנות רק עם אותיות שלו. בהינתן {::nomarkdown}\( \Sigma\){:/nomarkdown} כזה, כל הביטויים הרגולריים נבנים כך:
<ol>
	<li>{::nomarkdown}\( \emptyset\){:/nomarkdown} הוא ביטוי רגולרי.</li>
	<li>{::nomarkdown}\( \varepsilon\){:/nomarkdown} הוא ביטוי רגולרי.</li>
	<li>{::nomarkdown}\( \sigma\){:/nomarkdown} הוא ביטוי רגולרי, לכל {::nomarkdown}\( \sigma\in\Sigma\){:/nomarkdown}.</li>
	<li>אם {::nomarkdown}\( r_{1},r_{2}\){:/nomarkdown} הם ביטויים רגולריים כך גם {::nomarkdown}\( \left(r_{1}+r_{2}\right)\){:/nomarkdown}.</li>
	<li>אם {::nomarkdown}\( r_{1},r_{2}\){:/nomarkdown} הם ביטויים רגולריים כך גם {::nomarkdown}\( \left(r_{1}\cdot r_{2}\right)\){:/nomarkdown}.</li>
	<li>אם {::nomarkdown}\( r\){:/nomarkdown} הוא ביטוי רגולרי כך גם {::nomarkdown}\( \left(r^{*}\right)\){:/nomarkdown}.</li>
</ol>
שימו לב ש-{::nomarkdown}\( \emptyset\){:/nomarkdown} ו-{::nomarkdown}\( \varepsilon\){:/nomarkdown} כאן הם <strong>סימבולים</strong>. למשל, {::nomarkdown}\( \emptyset\){:/nomarkdown} <strong>אינו</strong> הקבוצה הריקה, בהקשר הזה - אנחנו לא אומרים משהו כמו "הקבוצה הריקה היא ביטוי רגולרי". אנחנו אומרים "הסימבול {::nomarkdown}\( \emptyset\){:/nomarkdown} הוא ביטוי רגולרי. היי, במקרה או שלא במקרה זה גם הסימבול שבו אנחנו משתמשים כדי לתאר את הקבוצה הריקה!". זו נקודה פדנטית ואם אתם לא מבינים למה אני מתעקש עליה, לא נורא.

החשיבות של הסוגריים היא בכך שהיא מבטיחה שלביטויים רגולריים תהיה <strong>קריאה יחידה</strong>. כלומר, בהינתן ביטוי רגולרי, תהיה רק דרך אחת "לפרש" אותו ולתת לו משמעות. למשל, בואו נסתכל על הביטוי נטול הסוגריים {::nomarkdown}\( a+b^{*}\){:/nomarkdown}. מה המשמעות שלו? בסדר הקדימויות הרגיל, הוא מתאר את השפה {::nomarkdown}\( \left\{ a\right\} \cup\left\{ b^{n}\ \|\ n\in\mathbb{N}\right\} \){:/nomarkdown}, אבל אפשר גם לקרוא אותו אחרת - בתור {::nomarkdown}\( \left(a+b\right)^{*}\){:/nomarkdown}, שמתאר את השפה {::nomarkdown}\( \left\{ a,b\right\} ^{*}\){:/nomarkdown} (שפת כל המילים מעל הא"ב {::nomarkdown}\( \left\{ a,b\right\} \){:/nomarkdown}). כלומר, הבעיה עם {::nomarkdown}\( a+b^{*}\){:/nomarkdown} היא שיש שתי דרכים שונות לקרוא אותו - גם בתור ביטוי מהצורה {::nomarkdown}\( r_{1}+r_{2}\){:/nomarkdown} (עם {::nomarkdown}\( r_{1}=a\){:/nomarkdown} ו-{::nomarkdown}\( r_{2}=b^{*}\){:/nomarkdown}) וגם בתור ביטוי מהצורה {::nomarkdown}\( r^{*}\){:/nomarkdown} (עם {::nomarkdown}\( r=a+b\){:/nomarkdown}), ובלי סוגריים אי אפשר לדעת מה מהן ה"נכונה" (סדר קדימויות פירושו, בשורה התחתונה, דרך מובלעת לשים סוגריים בביטוי).

הסוגריים מבטיחות שכל ביטוי רגולרי יהיה מאחת מהצורות 1-6, אבל <strong>רק</strong> מאחת מהן. לא ייתכן ביטוי רגולרי שהוא בו זמנית מהצורה {::nomarkdown}\( \left(r_{1}+r_{2}\right)\){:/nomarkdown} וגם {::nomarkdown}\( \left(r_{1}\cdot r_{2}\right)\){:/nomarkdown}, למשל. זה מאפשר לנו להגדיר סמנטיקה עבור כל אחת משש הצורות. בהינתן ביטוי רגולרי {::nomarkdown}\( r\){:/nomarkdown}, נסמן את השפה שהוא מגדיר ב-{::nomarkdown}\( L\left[r\right]\){:/nomarkdown}. היא נקבעת באופן הבא:
<ol>
	<li>{::nomarkdown}\( L\left[\emptyset\right]=\emptyset\){:/nomarkdown}</li>
	<li>{::nomarkdown}\( L\left[\varepsilon\right]=\left\{ \varepsilon\right\} \){:/nomarkdown}</li>
	<li>{::nomarkdown}\( L\left[\sigma\right]=\left\{ \sigma\right\} \){:/nomarkdown}</li>
	<li>{::nomarkdown}\( L\left[\left(r_{1}+r_{2}\right)\right]=L\left[r_{1}\right]\cup L\left[r_{2}\right]\){:/nomarkdown}</li>
	<li>{::nomarkdown}\( L\left[\left(r_{1}\cdot r_{2}\right)\right]=L\left[r_{1}\right]\cdot L\left[r_{2}\right]\){:/nomarkdown}</li>
	<li>{::nomarkdown}\( L\left[\left(r^{*}\right)\right]=L\left[r\right]^{*}\){:/nomarkdown}</li>
</ol>
שום דבר מפתיע, כמובן.

כאן נגמר הפורמליזם המתמטי. יש עוד אי-אילו קיצורים שנהוג להשתמש בהם גם בכתיב מתמטי רגיל - למשל, לכתוב {::nomarkdown}\( \Sigma\){:/nomarkdown} במקום לכתוב {::nomarkdown}\( \left(\sigma_{1}+\sigma_{2}+\dots+\sigma_{n}\right)\){:/nomarkdown} (כאשר {::nomarkdown}\( \Sigma=\left\{ \sigma_{1},\dots,\sigma_{n}\right\} \){:/nomarkdown}). כך למשל {::nomarkdown}\( \left(\Sigma\Sigma\right)^{*}\){:/nomarkdown} היא שפת כל המילים מאורך זוגי מעל {::nomarkdown}\( \Sigma\){:/nomarkdown}, אבל זהו בערך.
...
תשאלו - איך מריצים אוטומט אי דטרמיניסטי? התשובה - בקלות. אם יש לאוטומט {::nomarkdown}\( n\){:/nomarkdown} מצבים, מחזיקים תוך כדי ריצה וקטור בינארי מאורך {::nomarkdown}\( n\){:/nomarkdown} שאומר לכל מצב האם אחת מהריצות של האוטומט נמצאת כרגע במצב הזה - כבר דיברנו על זה בהקשר של סילוק אי דטרמיניזם. זו בעצם הרצה מובלעת של האוטומט הדטרמיניסטי בלי לייצר אותו במפורש. אפשר, כמובן, גם לבנות את האוטומט הדטרמיניסטי מתוך האי-דטרמיניסטי ולהריץ אותו, אבל הבניה הנאיבית לא יעילה כך שצריך בניה יותר חכמה שבתקווה לא תיצור אוטומט גדול מדי; ובפועל משתמשים בשיטה הזו פחות.
...
<p dir="ltr">/rulez!+1*/</p>
בקיצור - סגור קלייני מיוצג כרגיל על ידי כוכבית, בזמן שבפלוס משתמשים כדי להגיד "כמו סגור קלייני אבל לפחות פעם אחת" (כלומר {::nomarkdown}\( r^+=r\cdot r^*\){:/nomarkdown}). אלו סימונים סטנדרטיים גם בביטויים רגולריים תיאורטיים, רק ששם קל להבדיל בין פלוס של "או" ובין פלוס של סגור קלייני מאורך 1 לפחות, כי הפלוס השני הוא במעריך החזקה. מן הסתם כשכותבים במחשב ההבחנה הזו הולכת לאיבוד.
...
{::nomarkdown}\( L=\left\{ ww\ \|\ w\in\Sigma^{*}\right\}\){:/nomarkdown}
...
האם זה אומר שאפשר לעשות הכל עם ביטויים רגולריים? ובכן, לא ממש. אני לא מכיר דרך לקבל את{::nomarkdown}\( L=\left\{ a^{n}b^{n}\ \|\ n\in\mathbb{N}\right\}\){:/nomarkdown} עם ביטוי רגולרי אמיתי, למרות שאולי אפשר. והנה בעיה פרקטית. נניח שאנחנו רוצים לפרסר דף html אבל לא יודעים מה בדיוק לחפש כמו בדוגמה שנתתי למעלה, אלא רוצים פרסור כללי. הנה ביטוי רגולרי שמנסה לתפוס את כל מה שבתוך תג הטמל כלשהו, וגם לזכור מה התג עצמו:
