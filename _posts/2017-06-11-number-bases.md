...
הנה לכם שאלה: בבסיס שמונה מהו המספר \\(\left(80\right)_{8}\\)? התשובה היא שאין כזה מספר. אנחנו לא משתמשים בספרה 8 בבסיס שמונה. הספרות הן מ-0 עד 7. הסיבה לכך היא ש<strong>אין צורך</strong> בספרה 8. אם נבדוק מהו הביטוי החשבוני של \\(\left(80\right)_{8}\\) נקבל \\(8\cdot8^{1}=64\\); אבל כבר יש לי כתיב אחר למספר הזה, \\(\left(100\right)_{8}\\). אין לנו צורך בשתי שיטות כתיבה לאותו המספר ולכן אנחנו פשוט לא משתמשים בספרה ה"מיותרת" 8. באופן דומה, בבסיס עשר אין לנו <strong>ספרה</strong> אשר מתארת את המספר עשר. ובאופן כללי, אם יש לנו בסיס ספירה שמבוסס על המספר \\(n\\), אז נזדקק לספרות עבור כל המספרים מ-0 עד \\(n-1\\).
