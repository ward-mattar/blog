...
ואנו רוצים לגלות את ה-$latex t$ שעבורו $latex x_a(t)=x_b(t)$. כלומר:
...
מרחק בין שני מספרים ממשיים הוא פשוט הערך המוחלט של ההפרש שלהם. לכן, חישוב קצר יראה שהמרחק בין המספר 1 ובין האיבר ה-n-י של הסדרה הוא בדיוק $latex \frac{1}{2^n}$. ככל ש-n גדול יותר, כך המרחק הזה קטן יותר, והוא מתקרב לאפס עוד ועוד (אך אף פעם אינו נעלם לחלוטין). מכאן אפשר להסיק שגבול הסדרה הזו הוא 1. כאן נגמרת המתמטיקה והפילוסופיה מתחילה מחדש: מה זה אומר, שהגבול הוא 1? האם זה אומר שהסדרה "מגיעה" לשם? האם המספר הזה הוא האיבר ה"אחרון" של הסדרה? שוב - לא אכנס כעת לשאלות אלו. לעת עתה, די בכך שאנו <strong>מגדירים</strong> את סכום הטור האינסופי להיות הגבול של סדרת הסכומים החלקיים, ולכן עבור הטור שלעיל, התוצאה היא 1.
...
$latex \sum_{i=0}^k a_i10^i +\sum_{n=1}^\infty a_{-n}10^{-n}$
...
אם כן, מהו ...0.999? הפיתוח של המספר הזה לא כולל ספרות משמאל לנקודה העשרונית, והצורה של הספרות מימין מאוד פשוטה, כך שניתן לחשב את הטור שלו באופן כמעט ישיר. הטור הוא $latex \sum_{n=1}^\infty\frac{9}{10^n}=9\cdot \sum_{n=1}^\infty\frac{1}{10^n}$. למזלנו, הטור שקיבלנו הוא טור הנדסי, שמנתו $latex \frac{1}{10}$ והאיבר הראשון שלו $latex \frac{1}{10}$. כשאעסוק בהגדרה הפורמלית של הגבול אוכיח גם, פורמלית, כי לטור הנדסי אינסופי עם איבר ראשון $latex a_1$ ומנה $latex q$, סכום הטור הוא בדיוק $latex \frac{a_1}{1-q}$, בתנאי ש-$latex q$ בערכו המוחלט אינו גדול או שווה ל-1. מכאן נקבל שסכום הטור במקרה שלנו הוא:
