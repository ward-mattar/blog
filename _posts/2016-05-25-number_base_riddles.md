...
ייצוג בינארי זה אותו דבר, רק עם חזקות של 2, וספרות שהן או 0 או 1. למשל, המספר 1011, שבייצוג עשרוני אנחנו חושבים עליו בתור "אלף ואחד-עשרה", בייצוג בינארי מתאר את המספר {::nomarkdown}\( 2^{0}+2^{1}+2^{3}=1+2+8=11\){:/nomarkdown}. שיטת הייצוג הזו נוחה מאוד לשימוש במחשבים (פחות או יותר כל מידע במחשבים מיוצג בצורה בינארית, כשיורדים לרמת הייצוג הבסיסית ביותר).

הרעיון של הפתרון כעת הוא זה: נמספר את הבקבוקים מ-0 ועד 999, ונכתוב את המספר של כל אחד מהם בייצוג בינארי בעזרת 10 ספרות. למשל, בקבוק מס' אחד-עשרה ייכתב בתור 0000001011. נמספר את התלמידים מ-1 עד 10 (הפעם בלי הפרדה בין "אחדות" ו"עשרות"). כעת, התלמיד מס' {::nomarkdown}\( n\){:/nomarkdown} ישתה מכל הבקבוקים אשר כשכותבים את המספר שלהם בייצוג בינארי, הספרה ה-{::nomarkdown}\( n\){:/nomarkdown}-ית בהם היא 1 (כשאני מתחיל את הספירה מ-1). כלומר, מבקבוק מס' אחד-עשרה ישתו התלמידים מספר 1, מספר 2 ומספר 4. אחרי שתלמידים יתחילו להיעלם אפשר יהיה לשחזר את הבקבוק הנכון על ידי כתיבת מספר בייצוג בינארי, שבו במקום ה-{::nomarkdown}\( n\){:/nomarkdown} יש 0 אם התלמיד מספר {::nomarkdown}\( n\){:/nomarkdown} לא נעלם, ו-1 אם הוא כן נעלם. הסיבה שהפתרון הזה עובד היא שבעזרת 10 ספרות בינאריות אפשר לייצג את כל המספרים מ-0 עד 1023; זו הסיבה לבחירת המספרים הספציפית שלנו.
...
בפועל המקרה של ה-13 מטבעות עדיין לא פתיר ואסביר למה (וגם מה צריך לתקן כדי שהוא יהיה פתיר), אבל בואו ונתחיל עם פתרון המקרה של ה-12 מטבעות. כאן נכנס לתמונה <strong>ייצוג טרינרי</strong> של מספרים, כלומר ייצוג בבסיס 3. המספר 102, למשל, הוא אחד-עשרה בבסיס 3 ({::nomarkdown}\( 2\cdot3^{0}+0\cdot3^{1}+1\cdot3^{2}\){:/nomarkdown}). הטריק יהיה במציאת ייצוג חכם של כל מטבע באמצעות מספר בין 0 ל-26. בואו נתחיל עם מספור נאיבי <strong>שאינו עובד</strong> ונראה איך לתקן אותו. במספור הנאיבי נמספר את 12 המטבעות מ-1 עד 12. עכשיו, אפשר לייצג כל פתרון אפשרי לבעיה באמצעות מספר בין 0 ל-26; נאמר שאם {::nomarkdown}\( n\){:/nomarkdown} הוא בין 1 ו-12 אז {::nomarkdown}\( n\){:/nomarkdown} מייצג את הפתרון "המטבע {::nomarkdown}\( n\){:/nomarkdown} הוא המזוייף והוא קל יותר" ואם {::nomarkdown}\( n\){:/nomarkdown} הוא בין 14 ל-25 נאמר שהוא מייצג את הפתרון "המטבע {::nomarkdown}\( 26-n\){:/nomarkdown} הוא המזוייף והוא כבד יותר". עוד מעט נבין למה המקרה השני מוזר כזה.

עכשיו, נייצג את התוצאה של <strong>כל שקילה</strong> באמצעות מספר: 0 פירושו יהיה "צד ימין קל יותר", 1 יהיה "שני הצדדים שווים" ו-2 יהיה "צד שמאל קל יותר". אחרי שביצענו שלוש שקילות, קיבלנו סדרה של שלוש תשובות, למשל {::nomarkdown}\( 102\){:/nomarkdown} (לקרוא מימין לשמאל!) מייצגת את הסדרה "בשקילה הראשונה צד שמאל היה קל יותר, בשקילה השניה צד ימין היה קל יותר ובשקילה האחרונה הם היו שווים".
...
זו שאלה מצויינת. כבר החלטתי מה יעלה בגורלו של מטבע אחד-עשרה רק על פי התרחיש שבו הוא קל יותר. בואו נראה מה יקרה אם הוא המזוייף והוא כבד יותר: השקילה הראשונה תניב לי 0 במקום 2; השקילה השניה עדיין תניב לי 1; והשקילה השלישית תניב לי 2 במקום 0. כלומר, אני אקבל את הסדרה {::nomarkdown}\( 120\){:/nomarkdown} שמייצגת את המספר חמש עשרה, שאצלנו מתפרש בתור "המטבע המזוייף הוא אחד עשרה והוא כבד יותר". באופן כללי, המספר שאני אקבל במקרה שהמטבע כבד יותר ולא קל יותר הוא המספר שמתאים למקרה של מטבע קל יותר, אחרי שאני הופך כל 0 ל-2 וכל 2 ל-0 ולא משנה את ה-1-ים. מבחינה מתמטית, הפעולה הזו שקולה לביצוע <strong>חיסור</strong> מהמספר 222, שהוא 26. זה מסביר את הבחירה שלי קודם לומר שפתרון מספר {::nomarkdown}\( n\){:/nomarkdown}, כאשר {::nomarkdown}\( n\){:/nomarkdown} הוא בין 14 ל-26, מייצג את "המטבע {::nomarkdown}\( 26-n\){:/nomarkdown} מזוייף והוא כבד יותר".

רגע, אז מה הבעיה? רואים אותה מייד אם טורחים לבצע את החישוב הזה לכל מטבע, ומקבלים את רשימת כל השקילות שיבוצעו. זה מה שמקבלים:
<ul>
	<li>שקילה ראשונה: צד ימין {::nomarkdown}\( \left\{ 3,6,9,12\right\} \){:/nomarkdown}, צד שמאל {::nomarkdown}\( \left\{ 2,5,8,11\right\} \){:/nomarkdown}</li>
	<li>שקילה שניה: צד ימין {::nomarkdown}\( \left\{ 1,2,9,10,11\right\} \){:/nomarkdown} צד שמאל {::nomarkdown}\( \left\{ 6,7,8\right\} \){:/nomarkdown}</li>
	<li>שקילה שלישית: צד ימין {::nomarkdown}\( \left\{ 1,2,3,4,5,6,7,8\right\} \){:/nomarkdown}, צד שמאל {::nomarkdown}\( \left\{ \right\} \){:/nomarkdown}</li>
</ul>
משהו כאן בבירור משובש. הצרה היא שלא הכנסתי לתוך השיקולים שלי עד כה את הדרישה שבכל שקילה, <strong>תהיה אותה כמות מטבעות</strong> בשני צדי המאזניים. אנחנו רואים שבכל שלוש השקילות משתתפות 8 מטבעות, אבל המאזניים מאוד לא מאוזנים - רק בשקילה הראשונה יש 4 בכל צד. הפתרון לבעיה הזו יהיה להעביר חלק מהמטבעות צד. פורמלית זה אומר ש<strong>נשנה את הקידוד</strong> של המטבע. אם קודם המטבע מס' ארבע הייתה מיוצגת על ידי המספר הטרינרי {::nomarkdown}\( 011\){:/nomarkdown}, נחליף את הקידוד הזה עכשיו בהופכי שלו, וארבע יהיה מיוצג על ידי {::nomarkdown}\( 211\){:/nomarkdown}. הסיבה שבחרתי לקחת את ההופכי היא שזה מבטיח שלא יהיו לי התנגשויות - אף מטבע אחר לא קודד באמצעות 211. המשמעות של ההיפוך הזה היא שבכל שקילה שבה המטבע משתתף, אנחנו צריכים להעביר אותו לצד השני של המאזניים. בואו נעשה את ההיפוך הזה למספרים {::nomarkdown}\( 1,3,4,5\){:/nomarkdown}. נקבל את השקילות הבאות:
<ul>
	<li>שקילה ראשונה: צד ימין {::nomarkdown}\( \left\{ 5,6,9,12\right\} \){:/nomarkdown}, צד שמאל {::nomarkdown}\( \left\{ 2,3,8,11\right\} \){:/nomarkdown}</li>
	<li>שקילה שניה: צד ימין {::nomarkdown}\( \left\{ 2,9,10,11\right\} \){:/nomarkdown} צד שמאל {::nomarkdown}\( \left\{ 1,6,7,8\right\} \){:/nomarkdown}</li>
	<li>שקילה שלישית: צד ימין {::nomarkdown}\( \left\{ 2,6,7,8\right\} \){:/nomarkdown}, צד שמאל {::nomarkdown}\( \left\{ 1,3,4,5\right\} \){:/nomarkdown}</li>
</ul>
כך שכל מטבע בין 1 ל-12 מקודד על ידי הייצוג הטרינרי שלו למעט 1,3,4,5 שמיוצגים בהתאמה על ידי {::nomarkdown}\( 221,212,211,210\){:/nomarkdown}.

בואו נראה איך זה עובד. נניח שבשקילה הראשונה צד ימין היה קל יותר, בשניה המאזניים היו שווים ובשלישית צד שמאל היה קל יותר. קיבלנו את הסדרה 210, שמתארת את התשובה "המטבע המזוייף הוא 5 והוא קל יותר". האם זה תואם את השקילות? אכן! בשקילה הראשונה 5 היה בצד ימין, בשניה הוא לא השתתף, ובשלישית הוא היה בצד שמאל. ונניח שקיבלנו את סדרת השקילות {::nomarkdown}\( 201\){:/nomarkdown}. אין מספר שמיוצג על ידה, אבל ההיפוך שלה {::nomarkdown}\( 021\){:/nomarkdown} מייצג את מטבע שבע, ולכן המשמעות של {::nomarkdown}\( 201\){:/nomarkdown} היא "שבע כבד יותר". ואכן, בשקילה הראשונה שבע לא משתתף (ולכן יש איזון - התוצאה 1), בשניה הוא בצד שמאל (ולכן צד ימין קל יותר - התוצאה 0) ובשלישית הוא בצד ימין. לא קשה לבדוק שזה עובד לכל תריסר המטבעות - מצאנו את השקילות שלנו.

בשביל להבין למה אין לנו סיכוי לקבל פתרון עבור 13 מטבעות צריך לחשוב יותר בקפידה על מה בעצם עושה שקילה. שקילה <strong>פוסלת אפשרויות</strong> היפותטיות. למשל, אם בשקילה הראשונה אצלנו צד ימין היה קל מדי, נפסלה האפשרות ש-{::nomarkdown}\( 5\){:/nomarkdown} (שהיה בצד ימין) הוא כבד; נפסלה האפשרות ש-{::nomarkdown}\( 2\){:/nomarkdown} (שהיה בצד שמאל) הוא קל; ונפסלה האפשרות ש-{::nomarkdown}\( 7\){:/nomarkdown} (שלא השתתף) הוא כבד מדי או קל מדי. בצורה הזו נפסלות 16 אפשרויות מתוך ה-24 הקיימות ואנו נותרים רק עם 8. לא קשה לראות שכל תוצאה אפשרית של השקילה הראשונה תשאיר אותנו רק עם 8 אפשרויות, וזה מצויין.
...
תגידו, אולי נבצע שקילה אחרת בתור השקילה הראשונה. שקילה שמערבת {::nomarkdown}\( k\){:/nomarkdown} מטבעות פוסלת {::nomarkdown}\( 2k\){:/nomarkdown} אפשרויות אם היה איזון, ולכן אנחנו צריכים ש-{::nomarkdown}\( k\){:/nomarkdown} יהיה לפחות 9. מצד שני, אם אין איזון, אנחנו פוסלים בסך הכל {::nomarkdown}\( k\){:/nomarkdown} אפשרויות עבור המטבעות שנשקלו, ועוד {::nomarkdown}\( 2\left(13-k\right)\){:/nomarkdown} אפשרויות עבור המטבעות שלא נשקלו, כלומר בסך הכל {::nomarkdown}\( 26-k\){:/nomarkdown} אפשרויות, מה שאומר שאנחנו נשארים עם {::nomarkdown}\( k\){:/nomarkdown} אפשרויות. אם {::nomarkdown}\( k\){:/nomarkdown} הוא לפחות 10 אז שוב - טיעון עץ ההחלטה מביס אותנו. המסקנה היא שהשקילה הראשונה חייבת לערב בדיוק 9 מטבעות. הבעיה היא שאין דרך לחלק 9 מטבעות על שני צדי המאזניים כך שהם יהיו מאוזנים...

אם כן, 3 שקילות לא מספיקות עבור 13 מטבעות. אבל כאמור, אם יהיה לנו מטבע אחד נוסף שאנחנו יודעים שהוא תקין, אפשר יהיה להשתמש בו כדי לאזן מאזניים לא מאוזנים, וזה יפתור לנו את הבעיה. הפתרון הוא פשוט הרחבה של הפתרון עבור 12 שהצגתי קודם. בפתרון עבור 12, לא השתמשתי ב-0 בתור מספר חוקי של מטבע - בואו נשתמש בו עכשיו. מכיוון ש-0 בייצוג טרינרי הוא 000, זה אומר שאת המטבע הזה נשים בצד ימין של המאזניים בכל שלוש השקילות; אם כן, את המטבע התקין, שאסמן בתור {::nomarkdown}\( X\){:/nomarkdown}, צריך לשים תמיד בצד שמאל. קיבלנו את השקילות:
<ul>
	<li>שקילה ראשונה: צד ימין {::nomarkdown}\( \left\{ 0,5,6,9,12\right\} \){:/nomarkdown}, צד שמאל {::nomarkdown}\( \left\{ X,2,3,8,11\right\} \){:/nomarkdown}</li>
	<li>שקילה שניה: צד ימין {::nomarkdown}\( \left\{ 0,2,9,10,11\right\} \){:/nomarkdown} צד שמאל {::nomarkdown}\( \left\{ X,1,6,7,8\right\} \){:/nomarkdown}</li>
	<li>שקילה שלישית: צד ימין {::nomarkdown}\( \left\{ 0,2,6,7,8\right\} \){:/nomarkdown}, צד שמאל {::nomarkdown}\( \left\{ X,1,3,4,5\right\} \){:/nomarkdown}</li>
</ul>
והפתרון זהה לקודם.
