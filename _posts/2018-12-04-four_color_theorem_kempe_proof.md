...
<img class="alignnone wp-image-3703 size-full" src="https://gadial.net/wp-content/uploads/2018/12/planar-graph.png" alt="" width="562" height="662" />
...
<img class="alignnone wp-image-3704 size-full" src="https://gadial.net/wp-content/uploads/2018/12/dual-graph.png" alt="" width="568" height="611" />
...
עכשיו אפשר לחזור ולדבר על נוסחת אוילר. היא פשוטה להחריד: אם {::nomarkdown}\(V\){:/nomarkdown} הוא מספר הצמתים בגרף מישורי, {::nomarkdown}\(E\){:/nomarkdown} הוא מספר הקשתות ו-{::nomarkdown}\(F\){:/nomarkdown} הוא מספר הפאות שלו (כולל הפאה "החיצונית") אז {::nomarkdown}\(V-E+F=2\){:/nomarkdown}. הנוסחה התמימה הזו היא כנראה הנוסחה האהובה עלי במתמטיקה, וכבר <a href="https://gadial.net/2010/01/31/euler_formula_and_platonic_solids/">הקדשתי לה פוסט</a> (שעוסק בפאונים ולא בגרפים מישוריים אבל זה אותו הדבר; במקור אוילר אכן הוכיח אותה לפאונים).

קאמפ משתמש בנוסחת אוילר כדי להוכיח שבכל גרף מישורי חייב להיות צומת עם <strong>לכל היותר</strong> חמישה שכנים. החלק הזה במאמר של קאמפ הוא נכון לחלוטין והטיעון לא קשה; בואו נבין אותו עד הסוף. חוץ מנוסחת אוילר אצטרך עוד כמה נוסחאות שאפשר להסיק על גרפים מישוריים די בקלות. ראשית, {::nomarkdown}\(2E\ge3F\){:/nomarkdown}. למה? ובכן, בואו נעבור פאה-פאה ונספור את <strong>כל</strong> הקשתות שמרכיבות כל פאה. נקרא למספר הזה {::nomarkdown}\(X\){:/nomarkdown}. מה אני יודע על {::nomarkdown}\(X\){:/nomarkdown}? ראשית, מכיוון שכל פאה כוללת <strong>לפחות</strong> שלוש קשתות, הרי ש-{::nomarkdown}\(X\ge3F\){:/nomarkdown} (כי לכל אחת מ-{::nomarkdown}\(F\){:/nomarkdown} הפאות הוספנו ל-{::nomarkdown}\(X\){:/nomarkdown} לפחות 3). שנית, מכיוון שכל קשת שייכת בו זמנית בדיוק <strong>לשתי</strong> פאות, הרי ש-{::nomarkdown}\(X=2E\){:/nomarkdown} (כי ספרנו כל קשת פעמיים). קיבלנו {::nomarkdown}\(2E\ge3F\){:/nomarkdown} בלי להתאמץ בכלל.

עכשיו, בואו נסמן ב-{::nomarkdown}\(p_{i}\){:/nomarkdown} את מספר הצמתים בגרף שיש להם {::nomarkdown}\(i\){:/nomarkdown} שכנים. שימו לב שמספר השכנים של צומת שווה בדיוק למספר הקשתות שמחוברות אליו, ושבשיטת הספירה הזו כל קשת נספרת פעמיים (פעם אחת לכל אחד משני הצמתים שאליהם היא מחוברת). לכן אנחנו מקבלים את הנוסחה הבאה:

{::nomarkdown}\(2E=p_{1}+2p_{2}+3p_{3}+\dots\){:/nomarkdown}

הסכום באגף ימין הוא לא אינסופי כי החל ממקום מסויים {::nomarkdown}\(p_{i}=0\){:/nomarkdown} לכל {::nomarkdown}\(i\){:/nomarkdown} ולכן הסכום נעצר שם.

עכשיו, כמה צמתים יש בגרף בסך הכל? סכום מספר הצמתים עם שכן אחד, ומספר הצמתים עם שני שכנים, וכדומה:

{::nomarkdown}\(V=p_{1}+p_{2}+p_{3}+\dots\){:/nomarkdown}

עכשיו ננקוט בתעלול אלגברי נחמד: נכפול את המשוואה השניה ב-6 ונחסר ממנה את הראשונה. נקבל:

{::nomarkdown}\(6V-2E=5p_{1}+4p_{2}+3p_{3}+2p_{4}+p_{5}-p_{7}-2p_{8}-\dots\){:/nomarkdown}

או בסימון מקוצר:

{::nomarkdown}\(2\left(3V-E\right)=\sum_{i=1}^{\infty}\left(6-i\right)p_{i}\){:/nomarkdown}

עד עכשיו לא השתמשנו בכלל בנוסחת אוילר. כעת אפשר להשתמש בה: אני מסיק ממנה ש-{::nomarkdown}\(V=2+E-F\){:/nomarkdown}, מציב ב-{::nomarkdown}\(2\left(3V-E\right)\){:/nomarkdown} ומקבל:

{::nomarkdown}\(2\left(3V-E\right)=2\left(6+3E-3F-E\right)=12+2\left(2E-3F\right)\){:/nomarkdown}

מכיוון ש-{::nomarkdown}\(2E\ge3F\){:/nomarkdown}, הביטוי כולו הוא חיובי (הוא 12 ועוד משהו אי-שלילי). כלומר, גם הסכום {::nomarkdown}\(\sum_{i=1}^{\infty}\left(6-i\right)p_{i}\){:/nomarkdown} הוא חיובי. אבל יש בסכום הזה רק חמישה איברים חיוביים:

{::nomarkdown}\(5p_{1}+4p_{2}+3p_{3}+2p_{4}+p_{5}\){:/nomarkdown}

המסקנה היא שלפחות אחד מה-{::nomarkdown}\(p_{i}\){:/nomarkdown}-ים הללו צריך להיות חיובי, והוא מתאים בדיוק לצומת עם חמישה שכנים או פחות, כפי שרצינו.
...
<img class="alignnone wp-image-3705 size-full" src="https://gadial.net/wp-content/uploads/2018/12/color3.png" alt="" width="566" height="594" />
...
<img class="alignnone wp-image-3707 size-full" src="https://gadial.net/wp-content/uploads/2018/12/color4-1.png" alt="" width="757" height="505" />
...
<img class="alignnone wp-image-3708 size-full" src="https://gadial.net/wp-content/uploads/2018/12/color4-2.png" alt="" width="779" height="610" />
...
<img class="alignnone wp-image-3709 size-full" src="https://gadial.net/wp-content/uploads/2018/12/color4-3.png" alt="" width="765" height="668" />
...
<img class="alignnone wp-image-3711 size-full" src="https://gadial.net/wp-content/uploads/2018/12/color4-4-1.png" alt="" width="789" height="627" />
...
<img class="alignnone wp-image-3712 size-full" src="https://gadial.net/wp-content/uploads/2018/12/color4-5.png" alt="" width="971" height="782" />
...
<img class="alignnone wp-image-3714 size-full" src="https://gadial.net/wp-content/uploads/2018/12/color4-6-1.png" alt="" width="1023" height="780" />
...
<img class="alignnone wp-image-3715 size-medium" src="https://gadial.net/wp-content/uploads/2018/12/color5alt-282x300.png" alt="" width="282" height="300" />
...
<img class="alignnone wp-image-3716 size-full" src="https://gadial.net/wp-content/uploads/2018/12/color5.png" alt="" width="596" height="696" />
...
<img class="alignnone wp-image-3717 size-full" src="https://gadial.net/wp-content/uploads/2018/12/color5-chains.png" alt="" width="870" height="709" />
...
<img class="alignnone wp-image-3719 size-full" src="https://gadial.net/wp-content/uploads/2018/12/color5cross-1.png" alt="" width="770" height="786" />
