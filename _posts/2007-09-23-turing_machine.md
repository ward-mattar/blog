...
{::nomarkdown}\( q_{acc}\){:/nomarkdown} (acc מלשון "Accept"),

{::nomarkdown}\( q_{reg}\){:/nomarkdown} (rej מלשון "Reject"),

{::nomarkdown}\( q_1\){:/nomarkdown} (מלשון "ראיתי 1 או רק התחלתי לרוץ")

ו-{::nomarkdown}\( q_0\){:/nomarkdown} (מלשון "ראיתי 0").

המכונה תתחיל את ריצה במצב {::nomarkdown}\( q_1\){:/nomarkdown}, וכאן אני לבטח מבלבל אנשים שכבר מכירים את התחום ורגילים לכך שבדרך כלל המצב שמסומן כ-{::nomarkdown}\( q_0\){:/nomarkdown} הוא המצב ההתחלתי - אני מקווה שמטרתי כאן ברורה.

הנה דוגמה לאחד המעברים שבפונקצית המעברים: {::nomarkdown}\( \delta(q_1,0)=(q_0,0,R)\){:/nomarkdown}. כאן הפונקציה אומרת "בהינתן שאתה במצב {::nomarkdown}\( q_1\){:/nomarkdown} והראש הקורא קרא 0, עבור למצב {::nomarkdown}\( q_0\){:/nomarkdown}, כתוב 0 על הסרט במקום מה שהיה שם (כלומר, אל תשנה את מה שיש על הסרט) והזז את הראש הקורא צעד אחד ימינה". אני מקווה שלאחר הפרשנות הזו שאר המעברים יהיו ברורים (כאן b מציין "תא ריק בסרט"):

{::nomarkdown}\( \delta(q_1,1)=(q_1,1,R)\){:/nomarkdown}

{::nomarkdown}\( \delta(q_1,b)=(q_{rej},b,R)\){:/nomarkdown}

{::nomarkdown}\( \delta(q_0,0)=(q_0,0,R)\){:/nomarkdown}
{::nomarkdown}\( \delta(q_0,1)=(q_1,1,R)\){:/nomarkdown}

{::nomarkdown}\( \delta(q_0,b)=(q_{acc},b,R)\){:/nomarkdown}
