...
הבעיה היא לקבל כקלט פרמטרים שמגדירים סדרה חשבונית או הנדסית, ולהוציא את סכום הסדרה כפלט. בואו נזכיר מה אלו הסדרות הללו. בשני המקרים מדובר על סדרות של מספרים (ואני מניח בפתרונות שלי שאלו אפילו מספרים שלמים) מאורך סופי כלשהו שמסומן ב-{::nomarkdown}\( n\){:/nomarkdown}. את הסדרה עצמה אפשר לסמן, למשל, כך: {::nomarkdown}\( a_0, a_1, a_2, \dots, a_{n-1}\){:/nomarkdown}. שימו לב שאני מתחיל את מספור האיברים מ-0 ולכן האינדקס של האיבר האחרון הוא {::nomarkdown}\( n-1\){:/nomarkdown}. לסדרה <strong>חשבונית</strong> יש את התכונה המיוחדת שההפרש בין כל שני איברים סמוכים בה הוא מספר קבוע שמסומן ב-{::nomarkdown}\( d\){:/nomarkdown}; לסדרה <strong>הנדסית</strong> יש את התכונה שהמנה של כל שני איברים סמוכים בה היא מספר קבוע שמסומן ב-{::nomarkdown}\( q\){:/nomarkdown}. שלושת הפרמטרים של מספר האיברים בסדרה, האיבר הראשון בה וההפרש/מנה הקבוע מאפיינים באופן מוחלט את הסדרה: מאוד קל לראות שהאיבר מספר {::nomarkdown}\( i\){:/nomarkdown} בסדרה חשבונית הוא {::nomarkdown}\( a_0+id\){:/nomarkdown} והאיבר מספר {::nomarkdown}\( i\){:/nomarkdown} בסדרה הנדסית הוא {::nomarkdown}\( a_0q^{i}\){:/nomarkdown}.

עכשיו, מה שמבקשים מאיתנו למצוא הוא את <strong>סכום</strong> האיברים בסדרה, כלומר את {::nomarkdown}\( a_0+a_1+\dots+a_{n-1}\){:/nomarkdown}. בעולם האמיתי יש נוסחאות פשוטות שמחזירות את הסכום הזה: {::nomarkdown}\( n\left(a_0+\frac{d(n-1)}{2}\right)\){:/nomarkdown} עבור סדרה חשבונית ו-{::nomarkdown}\( a_0\frac{q^n-1}{q-1}\){:/nomarkdown} עבור סדרה הנדסית. לא קשה להוכיח שהנוסחאות הללו עובדות אבל לא אעשה את זה כרגע כי זה פוסט על תכנות ולא על טורים; הנקודה היא שאני בכלל לא הולך להשתמש בנוסחאות הללו. אני רוצה לחשב את הסכומים בדרך הקשה - לחשב איבר-איבר, ולקחת את הסכום שלהם.
